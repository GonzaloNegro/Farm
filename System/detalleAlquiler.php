...
<script>
    ...
    function validar_formulario(form){
        ...
        if (form.txtInversion.value == "") { 
            alerta('\"Monto estipulado\"'); form.txtInversion.focus(); return true; 
            }
            
            if(form.cmbNombre.value != "" && form.txtFechaInicio.value != "" && form.txtFechaCierre.value != "" && form.cmbMoneda.value != "" && form.tipoCambio.value != "" && form.txtInversion.value != ""){
...
<div class="centrado">
  <div class="centrado--titulo">
    <h1>Asignar detalles proyecto de Alquiler</h1>	
  </div>
  <div class="centrado--form">
    <form action="./agrDetalleAlquiler.php" name="form_mod" id="form_mod" method="POST" class="centrado--form--form">
      <div class="labelInput">
          <label for="name">Nombre del proyecto:</label>
          <select name="cmbNombre" required>
          <option value="" selected disabled="cmbnombre">-Seleccione una-</option>
          <?php
          $consulta= "SELECT * FROM proyectos WHERE id_Proyecto NOT IN (SELECT id_Proyecto FROM detallealquiler) AND id_TipoProyecto = '3'  AND id_EstadoProyecto != 3";
      ...
      <div class="labelInput">
          <label for="name">Monto estipulado:</label>
