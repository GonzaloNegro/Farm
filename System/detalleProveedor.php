...
{	
    include('../Modelo/Conexion.php');
	$sentencia =  "SELECT * FROM proveedores WHERE id_Proveedor='".$no_id."'";
	$resultado = mysqli_query($conexion, $sentencia);
	$filas = mysqli_fetch_assoc($resultado);
	return [
		$filas['id_Proveedor'],/*0*/
		$filas['proveedor'],
	];
}
$id = $consulta[0];
...
<div class="centrado">
    ...
    <div class="centrado--form">
        <form action="./modProveedor.php" name="form_mod" id="form_mod" method="POST" class="centrado--form--form">
            <div>
                <label for="id">Proveedor N°:</label>
                <input type="text" name="id" id="id" class="readonly" value="<?php echo $id; ?>" readonly>
            </div>
            <div>
                <label for="name">Nombre del proveedor:</label>
                <input type="text" name="prov" id="prov" value="<?php echo $prov;?>" required>
            </div>
